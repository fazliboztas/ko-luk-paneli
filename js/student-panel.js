...
function renderStudentWeeklyProgressChart() {
    ...
    new Chart(canvas, { type: "line", data: { labels: weekly.map(item => item.label), datasets: [{ label: "Çözülen soru", data: weekly.map(item => item.questions), borderColor: "#2563eb", backgroundColor: "rgb(37 99 235 / .12)", pointBackgroundColor: "#2563eb", pointRadius: 4, fill: true, tension: .3, yAxisID: "questions" }, { label: "Çalışma süresi (saat)", data: weekly.map(item => Number((item.minutes / 60).toFixed(2))), borderColor: "#16a34a", backgroundColor: "rgb(22 163 74 / .12)", pointBackgroundColor: "#16a34a", pointRadius: 4, fill: true, tension: .3, yAxisID: "duration" }] }, options: { responsive: true, maintainAspectRatio: false, interaction: { intersect: false, mode: "index" }, plugins: { legend: { position: "bottom" } }, scales: { x: { title: { display: true, text: "Program haftası" } }, questions: { beginAtZero: true, position: "left", title: { display: true, text: "Soru" } }, duration: { beginAtZero: true, position: "right", grid: { drawOnChartArea: false }, title: { display: true, text: "Saat" } } } } });
}
...
function studentPanelPage() {
    ...
    const totalMinutes = completedTasks.reduce((total, task) => total + (Number(task.duration) || 0), 0);
    const solvedQuestions = completedTasks.reduce((total, task) => total + (Number(task.solvedQuestions) || 0), 0);
